...
func _create_zone() -> void:
	...
	visual.color = Color(0.7, 0.2, 1.0, 0.4)
	minigame_area.add_child(visual)
	var label = Label.new()
	label.text = "Tapınak İçi"
	...
	minigame_area.body_entered.connect(func(body: Node2D):
		...
		if current != "xora_memory":
			...
			canvas.queue_free()
			return
		get_tree().call_deferred("change_scene_to_file", "res://scenes/minigames/xora_memory.tscn")
	)
	add_child(minigame_area)

	var shop1 = ColorRect.new()
	shop1.size = Vector2(50, 40)
	shop1.color = Color(0.5, 0.2, 0.7)
	shop1.position = Vector2(60, 100)
	add_child(shop1)
	var shop1_label = Label.new()
	shop1_label.text = "Bilge Rahip"
	...
	shop2.color = Color(0.8, 0.4, 0.6)
	shop2.position = Vector2(370, 100)
	add_child(shop2)
	var shop2_label = Label.new()
	shop2_label.text = "Mistik Tüccar"
