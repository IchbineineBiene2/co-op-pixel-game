...
func _create_zone() -> void:
	...
	visual.color = Color(1, 0.2, 0.2, 0.4)
	minigame_area.add_child(visual)
	var label = Label.new()
	label.text = "Dövüş Arenası"
	...
	minigame_area.body_entered.connect(func(body: Node2D):
		...
		if current != "glonar_fight":
			...
			canvas.queue_free()
			return
		get_tree().call_deferred("change_scene_to_file", "res://scenes/minigames/glonar_fight.tscn")
	)
	add_child(minigame_area)

	var shop1 = ColorRect.new()
	shop1.size = Vector2(50, 40)
	shop1.color = Color(0.55, 0.2, 0.15)
	shop1.position = Vector2(60, 100)
	add_child(shop1)
	var shop1_label = Label.new()
	shop1_label.text = "Silah Ustası"
	...
	shop2.color = Color(0.7, 0.4, 0.1)
	shop2.position = Vector2(370, 100)
	add_child(shop2)
	var shop2_label = Label.new()
	shop2_label.text = "Arena Koçu"
